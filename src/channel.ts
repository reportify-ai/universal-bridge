...
function isConfigured(cfg: OpenClawConfig): boolean {
  ...
  if (config.accounts && Object.keys(config.accounts).length > 0) {
    return Object.values(config.accounts).some((a: any) =>
      Boolean(a?.webhookUrl && a?.userId),
    );
  }
...
async function handleInboundMessage(params: {
  ...
  log?.info?.(
    `[Bridge] Inbound: from=${msg.userId} sessionKey=${ctx.SessionKey} agentRoute=${route.agentId}`,
  );
      ...
      await rt.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
        ...
        dispatcherOptions: {
          ...
          onError: (err: unknown, info: any) => {
            log?.error?.(
              `[Bridge] ${info?.kind ?? "unknown"} reply error: ${String(err)}`,
            );
          },
        },
      });
    log?.info?.(
      `[Bridge] Dispatch completed: queuedFinal=${result.queuedFinal} counts=${JSON.stringify(result.counts)}`,
    );
  } catch (err: any) {
...
export const bridgePlugin: BridgeChannelPlugin = {
  ...
  meta: {
    ...
    blurb:
      "A generic bidirectional webhook bridge for connecting any IM platform.",
  ...
  gateway: {
    startAccount: async (
      ...
      ctx.log?.info?.(
        `[${account.accountId}] Starting webhook listener on port ${port}...`,
      );
      ...
      if (portInUse) {
        ctx.log?.info?.(
          `[${account.accountId}] Port ${port} already in use, assuming previous instance is running. Skipping startup.`,
        );
        ctx.setStatus({
          ...ctx.getStatus(),
          running: true,
      ...
      if (abortSignal?.aborted) {
        ctx.log?.warn?.(
          `[${account.accountId}] Abort signal already active, skipping`,
        );
        ctx.setStatus({
          ...ctx.getStatus(),
          running: false,
      ...
      const server = http.createServer(async (req, res) => {
        ...
        try {
          ...
          if (config.secretKey) {
            ...
            if (
              !signature ||
              !verifySignature(body, signature, config.secretKey)
            ) {
          ...
          // Send "processing" hint if agent takes longer than 15s
          const processingTimer = setTimeout(async () => {
            try {
              await sendReply(config, msg.sessionId, "处理中...", ctx.log);
            } catch (err: any) {
              ctx.log?.error?.(
                `[${account.accountId}] Failed to send processing hint: ${err.message}`,
              );
            }
          }, 30000);
          ...
          handleInboundMessage({
            ...
            .catch((err: any) => {
              clearTimeout(processingTimer);
              ctx.log?.error?.(
                `[${account.accountId}] Async message handling error: ${err.message}`,
              );
            });
        } catch (err: any) {
          ctx.log?.error?.(
            `[${account.accountId}] Webhook handler error: ${err.message}`,
          );
          if (!res.headersSent) {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        server.listen(port, () => {
          ctx.log?.info?.(
            `[${account.accountId}] Webhook listener ready on port ${port}`,
          );
          resolve();
        });
      });
